...
class PaypalController extends Controller
{
    ...
    public function paypalReturn(Request $request){
        ...
        try {
            ...
            $msg_status = 'success';
            $payment_msg = "Welcome to CBETFlashcards.com, you payment was successful!";
    ...
    public function cancelUserAgreement(Request $request){
        $user = User::find($request->user_id);
        $agreement_id = $user->agreement_id;
        $plan = $user->plan;
        $status = $user->status;
        if($agreement_id && $plan != 'free' && $status == 'active') {
            $agreement = new Agreement();
            $agreement->setId($agreement_id);
            $agreementId = $agreement->getId();
            if($agreementId) {
				try{
					$cancelAgreementDetails = Agreement::get($agreementId, $this->apiContext);
					if ($agreement) {
						...
						try {
							...
							$user->delete();
							return back()->with('success', 'Account & Agreement Cancelled.');
						} catch (\Exception $ex) {
							$user->delete();
							return back()->with('error', 'User with no paypal agreement is deleted.');
						}
					} else {
						$user->delete();
						return back()->with('error', 'Paypal Agreement not found');
					}
				} catch (\Exception $ex) {
                    $user->delete();
                    return back()->with('error', 'User with no paypal agreement is deleted.');
            ...
            $user->save();
            $user->delete();
            return back()->with('error', 'Account cancelled');
        }
        $user->delete();
        return back()->with('error', 'Paypal Agreement not found');
